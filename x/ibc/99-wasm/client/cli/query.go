...
func GetCmdListCode(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "list-candidates",
		Short: "List candidate validity predicates",
		Long: strings.TrimSpace(fmt.Sprintf(`List candidate validity predicates:

Example:
$ %s query ibc wasm list-candidates`, version.ClientName)),
		Args: cobra.ExactArgs(0),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := client.NewContext().WithCodec(cdc)

			route := fmt.Sprintf("custom/ibc/%s/%s", types.QuerierRoute, types.QueryListCandidate)
...
func GetCmdListContractByCode(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Short: "Show candidate for given identifier",
		Long: strings.TrimSpace(fmt.Sprintf(`Show candidate validity predicates for given identifier:

Example:
$ %s query ibc wasm show-candidate [id]`, version.ClientName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%d", types.QuerierRoute, types.QueryShowCandidate, codeID)
			res, _, err := cliCtx.Query(route)
			if err != nil {
				return err
			}
...
func GetCmdQueryCode(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "show-code [id] [output filename]",
		Short: "Download validity predicate for given identifier",
		Long:  "Download validity predicate for given identifier",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%d", types.QuerierRoute, types.QueryGetCode, codeId)
			res, _, err := cliCtx.Query(route)
			if err != nil {
				return err
			}
			if len(res) == 0 {
				return fmt.Errorf("validity predicate not found for identifier '%d'", codeId)
			}
			...
			if len(code.Data) == 0 {
				return fmt.Errorf("unable to decode validity predicate for identifier '%d'", codeId)
			}
...
func GetCmdGetContractInfo(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%s", types.QuerierRoute, types.QueryGetContract, addr.String())
			res, _, err := cliCtx.Query(route)
			if err != nil {
				return err
			}
...
func GetCmdGetContractStateAll(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%s/%s", types.QuerierRoute, types.QueryGetContractState, addr.String(), keeper.QueryMethodContractStateAll)
			res, _, err := cliCtx.Query(route)
			if err != nil {
				return err
			}
...
func GetCmdGetContractStateRaw(cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%s/%s", types.QuerierRoute, types.QueryGetContractState, addr.String(), keeper.QueryMethodContractStateRaw)
...
func GetCmdGetContractStateSmart(cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/ibc/%s/%s/%s/%s", types.QuerierRoute, types.QueryGetContractState, addr.String(), keeper.QueryMethodContractStateSmart)
