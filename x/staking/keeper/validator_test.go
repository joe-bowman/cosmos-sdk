...
func TestSetValidator(t *testing.T) {
	...
	validator := types.NewValidator(valAddr, valPubKey, types.Description{}, "ONE")
...
func TestUpdateBondedValidatorsDecreaseCliff(t *testing.T) {
	...
	for i := 0; i < len(validators); i++ {
		...
		val := types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{Moniker: moniker}, fmt.Sprintf("DEN%d", i))
...
func GetValidatorSortingUnmixed(t *testing.T) {
	...
	for i, amt := range amts {
		validators[i] = types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{}, fmt.Sprintf("DEN%d", i))
...
func TestGetValidatorsEdgeCases(t *testing.T) {
	...
	for i, power := range powers {
		pool := keeper.GetPool(ctx)
		moniker := fmt.Sprintf("val#%d", int64(i))
		validators[i] = types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{Moniker: moniker}, fmt.Sprintf("DEN%d", i))
		tokens := sdk.TokensFromTendermintPower(power)
		validators[i], pool, _ = validators[i].AddTokensFromDel(pool, tokens)
		keeper.SetPool(ctx, pool)
...
func TestValidatorBondHeight(t *testing.T) {
	...
	// initialize some validators into the state
	var validators [3]types.Validator
	validators[0] = types.NewValidator(sdk.ValAddress(Addrs[0]), PKs[0], types.Description{}, "ONE")
	validators[1] = types.NewValidator(sdk.ValAddress(Addrs[1]), PKs[1], types.Description{}, "TWO")
	validators[2] = types.NewValidator(sdk.ValAddress(Addrs[2]), PKs[2], types.Description{}, "THREE")
...
func TestApplyAndReturnValidatorSetUpdatesAllNone(t *testing.T) {
	...
	for i, power := range powers {
		...
		validators[i] = types.NewValidator(valAddr, valPubKey, types.Description{}, fmt.Sprintf("DEN%d", i))
		tokens := sdk.TokensFromTendermintPower(power)
		validators[i], pool, _ = validators[i].AddTokensFromDel(pool, tokens)
		keeper.SetPool(ctx, pool)
	}
...
func TestApplyAndReturnValidatorSetUpdatesNewValidator(t *testing.T) {
	...
	validator = types.NewValidator(valAddr, valPubKey, types.Description{}, "TWO")
...
func TestApplyAndReturnValidatorSetUpdatesBondTransition(t *testing.T) {
	...
	for i, power := range powers {
		...
		validators[i] = types.NewValidator(valAddr, valPubKey, types.Description{Moniker: moniker}, fmt.Sprintf("DEN%d", i))
		tokens := sdk.TokensFromTendermintPower(power)
		validators[i], pool, _ = validators[i].AddTokensFromDel(pool, tokens)
		keeper.SetPool(ctx, pool)
		keeper.SetValidator(ctx, validators[i])
...
func TestUpdateValidatorCommission(t *testing.T) {
	...
	val1 := types.NewValidator(addrVals[0], PKs[0], types.Description{}, "ONE")
	val2 := types.NewValidator(addrVals[1], PKs[1], types.Description{}, "TWO")
