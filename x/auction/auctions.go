...
const (
	...
	BidExtensionDuration int64 = 0 // 30s /*1800 / 5*/ // roughly 30 mins, at 5s block time // 360
...
type BaseAuction struct {
	...
	TimeRemaining int64          `json:"time_remaining"` // Time remaing
	Status        string         `json:"auction_status"` // Auction Status (Active/Finished)
}
...
func (a BaseAuction) GetStatus() string { return a.Status }

func (a BaseAuction) SetStatus(status string) { a.Status = status }

func (a BaseAuction) GetTimeRemaining() int64 { return a.TimeRemaining }

func (a BaseAuction) SetTimeRemaining(blocks int64) { a.TimeRemaining = blocks }
...
func (a BaseAuction) String() string {
  ...
  Max End Time:      			%d
	Time Remaining:         %d`,
		a.GetID(), a.GetStatus(), a.GetLot(),
		a.GetBid(), a.GetBidder(), a.GetEndTime(),
		a.MaxEndTime, a.GetTimeRemaining(),
	)
}
...
func (a *ForwardAuction) PlaceBid(currentBlockHeight int64, bidder sdk.AccAddress, bid sdk.Coin) sdk.Error {
	...
	a.EndTime = min(max(a.EndTime, int64(currentBlockHeight+BidExtensionDuration)), int64(a.MaxEndTime)) // TODO is there a better way to structure these types?
